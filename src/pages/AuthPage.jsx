...
export function AccountPage() {
  ...
  const safeUser = user || { name: 'Гость', email: '' }
  const initials = safeUser.name.split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase()

  const [profileForm, setProfileForm] = useState({ name: safeUser.name, email: safeUser.email, phone: '', city: 'Санкт-Петербург' })
      ...
      <section className="account-content">
        ...
        {tab === 'inquiries' && (
          <div>
            ...
            {questions.length === 0 ? (
              <p style={{color:'var(--ink-3)', marginTop:24}}>У вас пока нет обращений. Задайте вопрос о работе на странице произведения.</p>
            ) : questions.map((th, i) => {
              const isOpen = expandedThread === i
              const img = th.artwork ? artworkImg(th.artwork) : ''
              return (
                <div key={i}>
                  <div className="thread" onClick={() => setExpandedThread(isOpen ? null : i)}>
                    {img ? (
                      <div className="thread-thumb" style={{backgroundImage:`url(${img})`}}/>
                    ) : (
                      <div className="thread-thumb" style={{background:'#F0EDE8'}}/>
                    )}
                    <div className="thread-body">
                      <div className="thread-title">{th.artworkTitle || 'Произведение'}</div>
                  ...
                  {isOpen && (
                    <div style={{padding:'0 0 32px 100px', borderBottom:'1px solid var(--line)'}}>
                      ...
                      <p style={{fontSize:13, color:'var(--ink-3)', padding:'16px 0'}}>Куратор галереи ответит в течение суток. Ответ придёт на {th.email}.</p>
                    </div>
                  )}
                </div>
              )
        ...
        {tab === 'cart' && (
          <div>
            ...
            {cart.length === 0 ? (
              ...
              <>
                {cart.map((w, i) => {
                  const isTicket = w.type === 'ticket'
                  const img = isTicket ? (w.coverImage || '') : artworkImg(w)
                  return (
                    <div key={w.id + '-' + i} className="cart-item">
                      <div className="cart-item-img"
                        style={{backgroundImage: img ? `url(${img})` : 'none', background: img ? '' : '#F0EDE8', cursor: isTicket ? 'default' : 'pointer'}}
                        onClick={!isTicket ? () => navigate('artwork', { id: w.id }) : undefined}/>
                      <div className="cart-item-info">
                        {isTicket ? (
                          <>
                            <div className="artist">Билет · Выставка</div>
                            <div className="title">{w.title}</div>
                            <div className="meta">{fmtDateRange(w.dateStart, w.dateEnd)}</div>
                          </>
                        ) : (
                          <>
                            <div className="artist">{w.artist?.name ?? ''}</div>
                ...
                <div className="cart-total">
                  <span className="label">Итого · {cart.length} {cart.length === 1 ? 'позиция' : cart.length < 5 ? 'позиции' : 'позиций'}</span>
