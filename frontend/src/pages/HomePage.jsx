...
import { SEED, localHero, localInterior, localExhibition, artworkImg, fmtRub, fmtDateRange } from '../data'
import { api } from '../api'
import { Icon, SectionHead, BgImage, GalleryImage } from '../components'
...
export default function HomePage() {
  ...
  useEffect(() => {
    api.exhibitions.list().then(data => {
      const visible = data.filter(e => e.status === 'current' || e.status === 'soon').slice(0, 3)
      setExhibitions(visible)
    }).catch(() => {})

    api.artworks.list({ limit: 8 }).then(res => {
  ...
  return (
    <main>
      ...
      <section className="hero">
        <BgImage className="hero-img" local={localHero('hero-main-2')} fallback={SEED('hero-main-2')}/>
        ...
        {exhibitions[0] && (
          <div className="hero-corner">Текущая выставка — «{exhibitions[0].title}»</div>
        )}
      </section>
      ...
      <section className="about-strip">
        ...
        <BgImage className="about-strip-right" local="/imagesMain/main_onas.jpg" fallback={SEED('interior-1')}/>
      </section>
          ...
          {exhibitions.map((exh, i) => {
            ...
            return (
              <div className="exh-card" key={exh.id} onClick={() => navigate('exhibitions')}>
                <BgImage className="exh-card-img" local={cover} fallback={SEED('exh-' + i)}>
                ...
                <div className="exh-card-body">
                  <div className="exh-card-dates">{fmtDateRange(exh.dateStart, exh.dateEnd)}</div>
      ...
      <section className="section tight" style={{paddingTop:0}}>
        ...
        <div className="masonry">
          {artworks.map((w, i) => {
            const img    = artworkImg(w)
            const artist = w.artist?.name ?? ''
            const h      = Math.round(800 * heights[i % heights.length])
            return (
              <div className="masonry-item" key={w.id} onClick={() => navigate('artwork', { id: w.id })}>
                {img ? (
                  <img src={img} alt={w.title}/>
                ) : (
                  <img src={`https://picsum.photos/seed/aspect-${w.id}/800/${h}`} alt={w.title}/>
                )}
                <div className="masonry-overlay">
                  <div className="masonry-overlay-title">{artist} — {w.title}</div>
